# Scenario 9c: full multi-train control with path planning using A*
...
# - six hubs attached to six trains (broadcasting position as color patterns, taking commands)
...
# - nine cities now instead of six
...
## works
# (1) to route one train, e.g. the CSX from LA to NYC,
# (2) to route the first train while a second train is stopped,
# (3) to route two trains to destinations that don't conflict, and
# (4) to route two trains in a way that can work, but the paths may conflict, for some cases
# (5) to route three trains CSX:LA->KC, UP KC->NYC, CN Calgary->Atlanta
# distances work now
# blocking should work too
# simplified
...
city_connectivity = {
    ...
    "ATLANTA": ["KANSAS_CITY", "NYC", "EGGOPOLIS"],
    "EGGOPOLIS": ["ATLANTA", "NEW_BRICKERTON"],
    "NEW_BRICKERTON": ["EGGOPOLIS", "BRICK_SCIENCE_CITY"],
    "BRICK_SCIENCE_CITY": ["NEW_BRICKERTON"]
}
...
track = {
    ...
    ("ATLANTA", "EGGOPOLIS"): {
        "switches": {},
        "patterns": {  # Patterns near Eggopolis
            "approach": (),
            "at_city": (Color.YELLOW)
        },
        "distance": 40,
        "reverse_for": ["ATLANTA"]
    },
    ("EGGOPOLIS", "ATLANTA"): {
        ...
        "distance": 40,
        "reverse_for": ["EGGOPOLIS"]
    },
    ("EGGOPOLIS", "NEW_BRICKERTON"): {
        "switches": {},
        "patterns": {  # Patterns near New Brickerton
            "approach": (),
            "at_city": (Color.RED)
        },
        "distance": 48,
    ...
    ("NEW_BRICKERTON", "EGGOPOLIS"): {
        ...
        "distance": 48,
        "reverse_for": ["NEW_BRICKERTON"]
    },
    ("NEW_BRICKERTON", "BRICK_SCIENCE_CITY"): {
        "switches": {},
        "patterns": {  # Patterns near Brick Science City
            "approach": (),
            "at_city": (Color.BLUE)
        },
        "distance": 72,
    ...
    ("BRICK_SCIENCE_CITY", "NEW_BRICKERTON"): {
        ...
        "distance": 72,
        "reverse_for": ["BRICK_SCIENCE_CITY"]
    }
}
    ...
    for channel in [SWITCH_STATUS_1, SWITCH_STATUS_2, SWITCH_STATUS_3, 
        ...
        if status and len(status) >= 2:
            ...
            if status_id not in processed_statuses:
                ...
                elif channel in [TRAIN_STATUS_CSX, TRAIN_STATUS_UP, TRAIN_STATUS_CN, TRAIN_STATUS_BNSF, TRAIN_STATUS_NS]:
                    train_name = status[1]
                    color_code = status[2]
...
while True:
    ...
    if cmd == 'q':
        ...
        if len(parts) >= 3:
            ...
            if train_name not in ["TRAIN_CSX", "TRAIN_UP", "TRAIN_CN", "TRAIN_BNSF", "TRAIN_NS"]:
                print("Invalid train name. Use: CSX, UP, CN, BNSF, or NS")
                continue
        ...
        while True:
            train = input("\nTrain name (CSX/UP/CN/BNSF/NS or blank to finish): ").strip().upper()
            if not train:
                break
                
            if train not in ["CSX", "UP", "CN", "BNSF", "NS"]:
                print("Invalid train name. Use: CSX, UP, CN, BNSF, or NS")
                continue
